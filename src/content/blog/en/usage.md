...
One of Zahit's most powerful assets is its clean, out-of-the-box support for bilingual content management. This guide will walk you through the folder architecture of your posts, formatting your post metadata (frontmatter), and utilizing Zahit’s smart language switcher to link Spanish and English posts.
...
- `src/content/blog/es/` — Holds your Spanish articles.
...
- **`translationKey`:** A custom unique text key linking the English and Spanish translations of the post together.

---

## 🌐 The Smart i18n Switching Mechanism

Zahit implements an advanced routing system inside the header language navigation toggle. When a visitor is reading an English post and clicks **"ES"**, the theme does not simply bounce them to the Spanish home page. 

Instead, it searches through your blog collection to find a Spanish post that shares the exact same `translationKey` value.

- **If an identical key is found:** It seamlessly redirects the reader directly to the Spanish translation page.
...
1. Create `src/content/blog/es/simplicidad.md` containing `translationKey: 'simplicity-post'`.
2. Create `src/content/blog/en/simplicity.md` containing `translationKey: 'simplicity-post'`.

With this configuration, readers can toggle back and forth between English and Spanish on the fly without ever losing their place.
